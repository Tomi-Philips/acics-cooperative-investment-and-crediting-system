...
@section('content')
    <div class="container px-4 py-6 mx-auto">
        <div class="flex flex-col items-start justify-between mb-6 md:flex-row md:items-center">
            <h1 class="mb-4 text-2xl font-bold text-gray-900 md:mb-0">Loan Report</h1>
        ...
        <div class="p-4 mb-6 bg-white rounded-lg shadow-md">
            <form action="{{ route('admin.reports.loans') }}" method="GET" class="flex flex-col gap-4 md:flex-row">
                <div>
                    <label for="status" class="block mb-1 text-sm font-medium text-gray-700">Loan Status</label>
                    <select id="status" name="status" class="block w-full border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 sm:text-sm">
                ...
                <div>
                    ...
                    <input type="date" id="start_date" name="start_date" value="{{ request('start_date') }}" class="block w-full border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 sm:text-sm">
                </div>
                <div>
                    <label for="end_date" class="block mb-1 text-sm font-medium text-gray-700">End Date</label>
                    <input type="date" id="end_date" name="end_date" value="{{ request('end_date') }}" class="block w-full border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 sm:text-sm">
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">Created Date</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($loans as $loan)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">{{ $loan->id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex items-center justify-center flex-shrink-0 w-10 h-10 bg-gray-100 rounded-full">
                                            <span class="text-sm font-medium text-gray-800">{{ strtoupper(substr($loan->user->name ?? 'U', 0, 1)) }}</span>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900">{{ $loan->user->name ?? 'Unknown User' }}</div>
                                            <div class="text-xs text-gray-500">{{ $loan->user->email ?? 'No email' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm font-medium text-gray-900 whitespace-nowrap">₦{{ number_format($loan->amount) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">{{ $loan->created_at->format('M d, Y') }}</td>
                            </tr>
                        @empty
                            <tr>
